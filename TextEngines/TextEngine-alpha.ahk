...
;   Gui, -DPIScale        
;RowHeight   := 150
      LV_Add("", A_LoopFileName, CTime(A_LoopFileTimeCreated), CountOfRows(Marker), UiRowSearch(SearchFor), UiRowContent(SearchFor), A_LoopFilePath)
      ;LV_Add("", A_LoopFileName, CTime(A_LoopFileTimeCreated))
      LV_ModifyCol(1,245)
      LV_ModifyCol(2,100)
      LV_ModifyCol(3,25)
...
;Msgbox 0x40000,, % "Search Completed" 
return
...
;   Gui, -DPIScale        
;RowHeight   := 150
      LV_Add("", A_LoopFileName, CTime(A_LoopFileTimeCreated), ExpRowSearch(SearchFor), ExpCountOfRows(SearchFor))
      ;LV_Add("", A_LoopFileName, CTime(A_LoopFileTimeCreated))
      LV_ModifyCol(1,215)
      LV_ModifyCol(2,100)
      LV_ModifyCol(3,140)
...
ExpRowContent(String)  {
  Loop, Read, %A_LoopFileFullPath%
    ...
    FileReadLine, RowInfo, %A_LoopFileFullPath%, A_Index
    ;RowInfo := chr(34) chr(34) chr(34) chr(34) RowInfo chr(34) chr(34) chr(34) chr(34)
     FullRowInfo := FullRowInfo RowInfo
     ;FileReadLine, OutputVar, Filename, LineNum
    }
   FullRowInfo := "`n" . FullRowInfo
    /*
    {
      ;MsgBox, 4,, "%String%" found on the line %A_Index%.`nContinue?
      ;IfMsgBox, No
...
ExpCountOfRows(String)  {
  Loop, Read, %A_LoopFileFullPath%
    If InStr(A_LoopReadLine, String)
   {
         ExpCountOfInstance += 1
         ;msgbox %A_LoopReadLine% %String%
        }
    ExpCountOfInstance := "`n" . ExpCountOfInstance
    Return ExpCountOfInstance
   }
