...
class UIA_Interface extends UIA_Base {
	...
	*/
	...
	; Tries to get the Chromium content from RenderWidgetHostHWND1 control
...
class UIA_Element extends UIA_Base {
	...
	DumpTextRecursive(maxDepth:=20, layer:="", useTreeWalker := False, cached := False) { ; This function might hang if the element has thousands of empty custom elements (e.g. complex webpage)
		local
		StrReplace(layer, ".",, dotcount)
		if (dotcount >= maxDepth)
			return ""
		...
		returnStr := ""
		for k, v in children {
			returnStr .= (cached ? v.CachedDumpText() : v.DumpText()) . "`n" . v.DumpTextRecursive(maxDepth, layer (layer == "" ? "" : ".") k, useTreeWalker, cached)
            ;returnStr .= layer . (layer == "" ? "" : ".") . k " " (cached ? v.CachedDumpText() : v.DumpText()) . "`n" . v.DumpTextRecursive(maxDepth, layer (layer == "" ? "" : ".") k, useTreeWalker, cached)
		}
        return returnStr
        ;return ""
	}
		; Returns info about the element: Name 
	DumpText() { 
		local
		global UIA_Enum
		return ((name := this.CurrentName) == "" ? "" :  name ) 
	}
	CachedDumpText() { 
		local
		global UIA_Enum
		return ((name := this.CachedName) == "" ? "" :  name )
	}
	; Returns info (Name) for all descendants of the element. maxDepth is the allowed depth of recursion, by default 20 layers. DO NOT call this on the root element!
	DumpAllText(maxDepth:=20) { 
		return (this.DumpText() .  "`n" . this.DumpTextRecursive(maxDepth))
	}
	; Requires caching for properties Name
	CachedDumpAllText(maxDepth:=20) { 
		return (this.CachedDumpText() .  "`n" . this.DumpTextRecursive(maxDepth,,,True))
	}
	/*
